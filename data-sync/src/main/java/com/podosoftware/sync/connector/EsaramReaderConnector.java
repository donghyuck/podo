...
public class EsaramReaderConnector implements ReadConnector {
	...
	String serviceName = properties.getProperty("serviceName"); // 서비스명:인터페이스정의서
								    // 참조(연계목록Sheet-행정공유센터
								    // 웹서비스명)
	String useSystemCode = properties.getProperty("useSystemCode"); // 사용자
									// 시스템코드:행정공유센터에
									// 문의(부처변경)
	String certServerId = properties.getProperty("certServerId"); // 부처 인증서
								      // ID(부처변경)
	...
	String userName = properties.getProperty("userName"); // 사용자 이름:연계담당 공무원
							      // 성명(부처변경)
	...
	sb.append("<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:typ=\"")
		.append(xmlnsUrl).append("\">").append("/n");
	...
	sb.append("         <typ:transactionUniqueId>").append(transactionUniqueId).append("</typ:transactionUniqueId>")
		.append("/n");
	...
	try {
	     ...
	     **************************************************/
	    // TODO 서비스 명이 바뀌면 수정해야함. (요청은 "<typ:get" + 서비스명 + ">" 임.
	    String original = requestXml.split("<typ:get" + serviceName + ">")[1]
		    .split("</typ:get" + serviceName + ">")[0];

	    if (isSetGpkiService()) {
		// 암호화
		byte[] encrypted = gpkiService.encrypt(original.getBytes(this.encoding),
			gpkiService.getTargetServerIdList());
		// 전자서명
	...
	try {
	    ...
	    // TODO 서비스 명이 바뀌면 수정해야함. (응답은 "<typ:get" + 서비스명 에서 "Service"가 빠진것 +
	    // "Response>" 임.
	    log.debug("checking response date !!");
	    String resEncodeStr = responseStr.split("<typ:get" + StringUtils.removeEnd(serviceName, "Service")
		    + "Response xmlns:typ=\"" + xmlnsUrl + "\">")[1]
		...
		if (isSetGpkiService()) {
			...
			// 복호화
			log.debug("ERROR XML RESPONSE : " + new String(gpkiService.decrypt(validated), httpClient.getCharset()));
