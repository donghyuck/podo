...
public class SecureCompetencyMgmtController {
    ...
    @RequestMapping(value = "/mgmt/competency/codeset/group/list.json", method = { RequestMethod.POST,
	    RequestMethod.GET })
    @ResponseBody
		...
		if (type == 1) {
			...
			if (row != null) {
				...
				if (!job_item.getItems().containsKey(competencyUnitCode)) {
				    job_item.getItems().put(competencyUnitCode, new CodeItem("competency",
					    competencyUnitTitle, competencyUnitCode, competencyLevel));
				}
			    }
			}
		    }
	...
	Classification classify = new DefaultClassification(classifiyType, classifiedMajorityId, classifiedMiddleId,
	...
	totalCount = competencyManager.getCompetencyCount(company, competencyGroupCodeToUse, competencyLevel,
		competencyNameToUse, classify, jobId);
	if (totalCount > 0) {
	    if (pageSize > 0)
		items = competencyManager.findCompetency(company, competencyGroupCodeToUse, competencyLevel,
			competencyNameToUse, classify, jobId, startIndex, pageSize);
	    else
		items = competencyManager.findCompetency(company, competencyGroupCodeToUse, competencyLevel,
			competencyNameToUse, classify, jobId);
	}
	...
	for (Competency competency : items) {
	    if (competency.getCompetencyType() == CompetencyType.JOB
		    || competency.getCompetencyType() == CompetencyType.NONE) {
		try {
    ...
    @ResponseBody
    public Competency createCompetency(@RequestBody DefaultCompetency competency)
    ...
    @ResponseBody
    public Competency updateCompetency(@RequestBody DefaultCompetency competency)
	    throws CompetencyNotFoundException, CompetencyAlreadyExistsException {
	User user = SecurityHelper.getUser();
	Company company = user.getCompany();
    ...
    @ResponseBody
    public Job updateJob(@RequestBody DefaultJob job) throws JobNotFoundException {
	Job jobToUse = job;
    ...
    @ResponseBody
    public EssentialElement updateEssentialElement(@RequestBody DefaultEssentialElement element)
    ...
    @ResponseBody
    public PerformanceCriteria updatePerformanceCriteria(@RequestBody DefaultPerformanceCriteria performanceCriteria)
	    throws PerformanceCriteriaNotFoundException {

	if (performanceCriteria.getObjectType() < 1 || performanceCriteria.getObjectId() < 1)
	    throw new IllegalArgumentException(
		    "PerformanceCriteria not allowed for objectType[" + performanceCriteria.getObjectType() + "] ");
    ...
    @ResponseBody
    public Ability updateAbility(@RequestBody DefaultAbility ability) throws AbilityNotFoundException {
    ...
    @ResponseBody
    public List<Ability> updateAbilities(@RequestBody List<DefaultAbility> abilities) throws AbilityNotFoundException {

	List<Ability> listToUse = new ArrayList<Ability>();
	for (Ability ability : abilities) {
	    if (ability.getObjectType() < 1 || ability.getObjectId() < 1)
		throw new IllegalArgumentException(
			"Ability not allowed for objectType[" + ability.getObjectType() + "] ");
    ...
    @ResponseBody
    public List<Ability> removeAbilities(@RequestBody List<DefaultAbility> abilities) throws AbilityNotFoundException {
    ...
    @ResponseBody
    public AssessmentPlan createAssessmentPlanByCreatePlan(@RequestBody AssessmentCreatePlan plan)
    ...
    @ResponseBody
    public AssessmentPlan updateAssessmentPlan(@RequestBody DefaultAssessmentPlan assessment)
	    throws AssessmentPlanNotFoundException, AssessmentSchemeNotFoundException {
    ...
    @ResponseBody
    public RatingScheme updateRatingScheme(@RequestBody DefaultRatingScheme ratingScheme)
    ...
    @ResponseBody
    public AssessmentScheme updateAssessmentScheme(@RequestBody DefaultAssessmentScheme assessmentScheme)
