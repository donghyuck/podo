...
public class JdbcAssessmentDao extends ExtendedJdbcDaoSupport implements AssessmentDao {
	...
	return extendedPropertyDao.getProperties(ratingSchemePropertyTableName, ratingSchemePropertyPrimaryColumnName,
		ratingSchemeId);
    }

    public void setRatingSchemeProperties(long ratingSchemeId, Map<String, String> props) {
	extendedPropertyDao.updateProperties(ratingSchemePropertyTableName, ratingSchemePropertyPrimaryColumnName,
		ratingSchemeId, props);
    }

    public void deleteRatingSchemeProperties(long ratingSchemeId) {
	extendedPropertyDao.deleteProperties(ratingSchemePropertyTableName, ratingSchemePropertyPrimaryColumnName,
		ratingSchemeId);
    }

    public Map<String, String> getAssessmentSchemeProperties(long assessmentSchemeId) {
	return extendedPropertyDao.getProperties(assessmentSchemePropertyTableName,
		assessmentSchemePropertyPrimaryColumnName, assessmentSchemeId);
    }

    public void setAssessmentSchemeProperties(long assessmentSchemeId, Map<String, String> props) {
	extendedPropertyDao.updateProperties(assessmentSchemePropertyTableName,
		assessmentSchemePropertyPrimaryColumnName, assessmentSchemeId, props);
    }

    public void deleteAssessmentSchemeProperties(long assessmentSchemeId) {
	extendedPropertyDao.deleteProperties(assessmentSchemePropertyTableName,
		assessmentSchemePropertyPrimaryColumnName, assessmentSchemeId);
    }

    public Map<String, String> getAssessmentProperties(long assessmentId) {
	return extendedPropertyDao.getProperties(assessmentPlanPropertyTableName,
		assessmentPlanPropertyPrimaryColumnName, assessmentId);
    }

    public void setAssessmentProperties(long assessmentId, Map<String, String> props) {
	extendedPropertyDao.updateProperties(assessmentPlanPropertyTableName, assessmentPlanPropertyPrimaryColumnName,
		assessmentId, props);
    }

    public void deleteAssessmentProperties(long assessmentId) {
	extendedPropertyDao.deleteProperties(assessmentPlanPropertyTableName, assessmentPlanPropertyPrimaryColumnName,
		assessmentId);
    }
	...
	return getExtendedJdbcTemplate().queryForObject(
		getBoundSql("COMPETENCY_ACCESSMENT.COUNT_RATING_SCHEME_BY_OBJECT_TYPE_AND_OBJECT_ID").getSql(),
	...
	try {
	    scheme = getExtendedJdbcTemplate().queryForObject(
		    getBoundSql("COMPETENCY_ACCESSMENT.SELECT_RATING_SCHEME_BY_ID").getSql(), ratingSchemeMapper,
		...
		log.warn((new StringBuilder()).append("Multiple occurrances of the same ratingScheme ID found: ")
			.append(ratingSchemeId).toString());
		throw e;
	    }
	} catch (DataAccessException e) {
	    String message = (new StringBuilder()).append("Failure attempting to load ratingScheme by ID : ")
		    .append(ratingSchemeId).append(".").toString();
	    log.fatal(message, e);
	}
	return scheme;
    }
	...
	return getExtendedJdbcTemplate().query(
		...
		this.ratingLevelMapper, new SqlParameterValue(Types.NUMERIC, ratingSchemeId));
    }
	...
	if (inserts.size() > 0) {
	    getExtendedJdbcTemplate().batchUpdate(getBoundSql("COMPETENCY_ACCESSMENT.INSERT_RATING_LEVEL").getSql(),
	...
	if (updates.size() > 0) {
	    getExtendedJdbcTemplate().batchUpdate(getBoundSql("COMPETENCY_ACCESSMENT.UPDATE_RATING_LEVEL").getSql(),
		    new BatchPreparedStatementSetter() {
			public void setValues(PreparedStatement ps, int i) throws SQLException {
	...
	return getExtendedJdbcTemplate().queryForObject(
		getBoundSql("COMPETENCY_ACCESSMENT.COUNT_ASSESSMENT_PLAN_BY_OBJECT_TYPE_AND_OBJECT_ID").getSql(),
		Integer.class, new SqlParameterValue(Types.NUMERIC, objectType),
		new SqlParameterValue(Types.NUMERIC, objectId));
    }
	...
	try {
	    scheme = getExtendedJdbcTemplate().queryForObject(
		    getBoundSql("COMPETENCY_ACCESSMENT.SELECT_ASSESSMENT_PLAN_BY_ID").getSql(), assessmentPlanMapper,
	    ...
	    if (e.getActualSize() > 1) {
		log.warn((new StringBuilder()).append("Multiple occurrances of the same assessment ID found: ")
			.append(assessmentId).toString());
		throw e;
	    }
	} catch (DataAccessException e) {
	    String message = (new StringBuilder()).append("Failure attempting to load assessment by ID : ")
		    .append(assessmentId).append(".").toString();
	    log.fatal(message, e);
	}
	return scheme;
    }
	...
	return getExtendedJdbcTemplate().queryForObject(
		getBoundSql("COMPETENCY_ACCESSMENT.COUNT_ASSESSMENT_SCHEME_BY_OBJECT_TYPE_AND_OBJECT_ID").getSql(),
		Integer.class, new SqlParameterValue(Types.NUMERIC, objectType),
		new SqlParameterValue(Types.NUMERIC, objectId));
    }
	...
	try {
	    scheme = getExtendedJdbcTemplate().queryForObject(
		    ...
		    assessmentSchemeMapper, new SqlParameterValue(Types.NUMERIC, assessmentSchemeId));

	    scheme.setProperties(getAssessmentSchemeProperties(assessmentSchemeId));

	} catch (IncorrectResultSizeDataAccessException e) {
	    if (e.getActualSize() > 1) {
		log.warn((new StringBuilder()).append("Multiple occurrances of the same assessmentScheme ID found: ")
			.append(assessmentSchemeId).toString());
		throw e;
	    }
	} catch (DataAccessException e) {
	    String message = (new StringBuilder()).append("Failure attempting to load assessmentScheme by ID : ")
		    .append(assessmentSchemeId).append(".").toString();
	    log.fatal(message, e);
	}
	return scheme;
    }
	...
	if (assessmentScheme.getAssessmentSchemeId() > 0) {
	    ...
	    getJdbcTemplate().update(getBoundSql("COMPETENCY_ACCESSMENT.INSERT_ASSESSMENT_SCHEME").getSql(),
		    ...
		    new SqlParameterValue(Types.TIMESTAMP, assessmentScheme.getModifiedDate()));
	    if (assessmentScheme.getProperties().size() > 0) {
		setAssessmentSchemeProperties(assessmentScheme.getAssessmentSchemeId(),
			assessmentScheme.getProperties());
	    }
	}
    }
	...
	return getExtendedJdbcTemplate().queryForList(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_ASSESSMENT_JOB_SELECTION_IDS_BY_OBJECT_TYPE_AND_OBJECT_ID")
			.getSql(),
		Long.class, new SqlParameterValue(Types.NUMERIC, objectType),
		new SqlParameterValue(Types.NUMERIC, objectId));
    }
	...
	return getExtendedJdbcTemplate().queryForObject(
		getBoundSql("COMPETENCY_ACCESSMENT.COUNT_ASSESSMENT_JOB_SELECTION_IDS_BY_OBJECT_TYPE_AND_OBJECT_ID")
			.getSql(),
		Integer.class, new SqlParameterValue(Types.NUMERIC, objectType),
		new SqlParameterValue(Types.NUMERIC, objectId));
    }
	...
	try {
	    sjb = getExtendedJdbcTemplate().queryForObject(
		    ...
		    jobSelectionMapper, new SqlParameterValue(Types.NUMERIC, subjectId));
	} catch (IncorrectResultSizeDataAccessException e) {
	    if (e.getActualSize() > 1) {
		log.warn((new StringBuilder()).append("Multiple occurrances of the same JobSelection ID found: ")
			.append(subjectId).toString());
		throw e;
	    }
	} catch (DataAccessException e) {
	    String message = (new StringBuilder()).append("Failure attempting to load JobSelection by ID : ")
	...
	getExtendedJdbcTemplate().batchUpdate(getBoundSql("COMPETENCY_ACCESSMENT.REMOVE_ASSESSMENT_SUBJECT").getSql(),
		new BatchPreparedStatementSetter() {
		    public void setValues(PreparedStatement ps, int i) throws SQLException {
	...
	return getExtendedJdbcTemplate().queryForList(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_ASSESSMENT_SUBJECT_IDS_BY_OBJECT_TYPE_AND_OBJECT_ID")
			.getSql(),
		Long.class, new SqlParameterValue(Types.NUMERIC, objectType),
		new SqlParameterValue(Types.NUMERIC, objectId));
    }
	...
	return getExtendedJdbcTemplate().queryForObject(
		getBoundSql("COMPETENCY_ACCESSMENT.COUNT_ASSESSMENT_SUBJECT_IDS_BY_OBJECT_TYPE_AND_OBJECT_ID").getSql(),
		Integer.class, new SqlParameterValue(Types.NUMERIC, objectType),
		new SqlParameterValue(Types.NUMERIC, objectId));
    }
	...
	try {
	    jobSel = getExtendedJdbcTemplate().queryForObject(
		    getBoundSql("COMPETENCY_ACCESSMENT.SELECT_ASSESSMENT_SUBJECT_BY_ID").getSql(), subjectMapper,
		    new SqlParameterValue(Types.NUMERIC, selectionId));
	} catch (IncorrectResultSizeDataAccessException e) {
	    if (e.getActualSize() > 1) {
		log.warn((new StringBuilder()).append("Multiple occurrances of the same subject ID found: ")
			.append(selectionId).toString());
		throw e;
	    }
	} catch (DataAccessException e) {
	    String message = (new StringBuilder()).append("Failure attempting to load subject by ID : ")
		    .append(selectionId).append(".").toString();
	    log.fatal(message, e);
	}
	return jobSel;
    }
	...
	return getExtendedJdbcTemplate().queryForList(
		getBoundSqlWithAdditionalParameter("COMPETENCY_ACCESSMENT.SELECT_ASSESSMENT_PLAN_IDS_BY_USER", map)
			.getSql(),
		Long.class);
    }
	...
	if (StringUtils.isNotBlank(state))
	    map.put("state", state);
	return getExtendedJdbcTemplate().queryForList(
		getBoundSqlWithAdditionalParameter(
			"COMPETENCY_ACCESSMENT.SELECT_ASSESSMENT_IDS_BY_PLAN_AND_USER_AND_STATE", map).getSql(),
		Long.class, new SqlParameterValue(Types.NUMERIC, user.getUserId()));
    }
	...
	return getExtendedJdbcTemplate()
		.queryForObject(
			getBoundSqlWithAdditionalParameter(
				"COMPETENCY_ACCESSMENT.COUNT_ASSESSMENT_BY_PLAN_AND_USER_AND_STATE", map).getSql(),
	...
	try {
	    assessment = getExtendedJdbcTemplate().queryForObject(
		    getBoundSql("COMPETENCY_ACCESSMENT.SELECT_ASSESSMENT_BY_ID").getSql(), assessmentMapper,
		    new SqlParameterValue(Types.NUMERIC, assessmentId));
	} catch (IncorrectResultSizeDataAccessException e) {
	    if (e.getActualSize() > 1) {
		log.warn((new StringBuilder()).append("Multiple occurrances of the same Assessment ID found: ")
	...
	return getExtendedJdbcTemplate().query(
		...
		assessmentQuestionMapper, new SqlParameterValue(Types.NUMERIC, jobId),
    ...
    public List<AssessedEssentialElementScoreItem> getAssessedEssentialElementScoreAverageByPlanAndJob(
	    long assessmentPlanId, long jobId, int jobLevel) {
	return getExtendedJdbcTemplate().query(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_ASSESSMENT_OTHERS_AVG").getSql(),
		assessedEssentialElementScoreMapper, new SqlParameterValue(Types.NUMERIC, assessmentPlanId),
		new SqlParameterValue(Types.NUMERIC, jobId), new SqlParameterValue(Types.NUMERIC, jobLevel));

    }

    public List<AssessedEssentialElementScore> getAssessedEssentialElementSummaries(long assessmentId) {
	return getExtendedJdbcTemplate().query(getBoundSql("COMPETENCY_ACCESSMENT.SELECT_ASSESSMENT_SUMMARY").getSql(),
		assessedEssentialElementSummaryMapper, new SqlParameterValue(Types.NUMERIC, assessmentId));
    }
     ...
     * 
     * 
     * CREATE TABLE CA_ASSESSED_SCORE ( ASSESSMENT_ID INTEGER NOT NULL,
     * CANDIDATE_ID INTEGER NOT NULL, ASSESSOR_ID INTEGER NOT NULL,
     * COMPETENCY_ID INTEGER NOT NULL, ESSENTIAL_ELEMENT_ID INTEGER NOT NULL,
     * PERFORMANCE_CRITERIA_ID INTEGER NOT NULL, CREATION_DATE TIMESTAMP DEFAULT
     * SYSTIMESTAMP NOT NULL, MODIFIED_DATE TIMESTAMP DEFAULT SYSTIMESTAMP NOT
     * NULL,
     * 
     * 
     * @param answers
	...
	if (inserts.size() > 0) {
	    getExtendedJdbcTemplate().batchUpdate(getBoundSql("COMPETENCY_ACCESSMENT.INSERT_ASSESSMENT_SCORE").getSql(),
		    new BatchPreparedStatementSetter() {
			public void setValues(PreparedStatement ps, int i) throws SQLException {
