...
public class JobSelectionsJsonDeserializer extends JsonDeserializer<List<JobSelection>> {
	...
	public List<JobSelection> deserialize(JsonParser jsonParser, DeserializationContext ctxt)
		...
		while( iter.hasNext() ){
			...
			list.add(
				new JobSelection(
					child.get("selectionId").asLong(-1L),
					child.get("objectType").asInt(0),
					child.get("objectId").asLong(-1L),
					child.get("classifyType").asLong(-1L),	
					child.get("classifiedMajorityId").asLong(-1L),	
					child.get("classifiedMiddleId").asLong(-1L),	
					child.get("classifiedMinorityId").asLong(-1L),	
					child.get("jobId").asLong(-1L)	
			));
		}
		return list;
	}
