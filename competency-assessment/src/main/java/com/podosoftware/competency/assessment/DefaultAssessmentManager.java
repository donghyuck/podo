...
public class DefaultAssessmentManager implements AssessmentManager {
	...
	if (scheme == null) {
	    ...
	    List<JobSelection> selections = getJobSelections(scheme.getModelObjectType(),
		    scheme.getAssessmentSchemeId());
	...
	if (selection.getClassifiedMajorityId() > 0 && StringUtils.isEmpty(selection.getClassifiedMajorityName())) {
	    try {
		selection.setClassifiedMajorityName(
			codeSetManager.getCodeSet(selection.getClassifiedMajorityId()).getName());
	    } catch (CodeSetNotFoundException e) {
	    }
	}
	if (selection.getClassifiedMiddleId() > 0 && StringUtils.isEmpty(selection.getClassifiedMiddleName())) {
	    try {
		selection.setClassifiedMiddleName(
			codeSetManager.getCodeSet(selection.getClassifiedMiddleId()).getName());
	    } catch (CodeSetNotFoundException e) {
	    }
	}
	if (selection.getClassifiedMinorityId() > 0 && StringUtils.isEmpty(selection.getClassifiedMinorityName())) {
	    try {
		selection.setClassifiedMinorityName(
			codeSetManager.getCodeSet(selection.getClassifiedMinorityId()).getName());
	    } catch (CodeSetNotFoundException e) {
	    }
	}
	...
	if (sbj.getSubjectObjectType() == 1 && sbj.getSubjectObjectId() > 0
		&& StringUtils.isEmpty(sbj.getSubjectObjectName())) {
	    try {
	...
	if (isNew) {
		...
		jobSelections.add(new JobSelection(-1L, assessment.getModelObjectType(), assessment.getAssessmentId(),
			js.getClassifyType(), js.getClassifiedMajorityId(), js.getClassifiedMiddleId(),
			js.getClassifiedMinorityId(), js.getJobId()));
	    }
	    for (Subject sbj : assessment.getSubjects()) {
		subjects.add(new Subject(-1L, assessment.getModelObjectType(), assessment.getAssessmentId(),
			sbj.getSubjectObjectType(), sbj.getSubjectObjectId()));
	    }
	} else {
	...
	List<Assessment> assessments = loadAssessments(
		assessmentDao.getUserAssessmentIds(user, assessmentPlan.getAssessmentId(), null));
    ...
    public Assessment getAssessment(long assessmentId)
	    throws AssessmentNotFoundException, AssessmentPlanNotFoundException {
	...
	if (assessment == null) {
	    ...
	    /**
	     * AssessmentPlan plan; if(
	     * assessment.getAssessmentPlan().getAssessmentId() > 0){ try { plan
	     * =
	     * getAssessmentPlan(assessment.getAssessmentPlan().getAssessmentId(
	     * )); assessment.setAssessmentPlan(plan); } catch
	     * (AssessmentPlanNotFoundException e) { } } if(
	     * assessment.getJob().getJobId() > 0 ){ try { Job job =
	     * jobManager.getJob(assessment.getJob().getJobId());
	     * assessment.setJob(job); } catch (JobNotFoundException e) {
	     * e.printStackTrace(); } };
	     */
	    // if( assessment.getJob().getJobId() > 0 &&
	    // assessment.getJobLevel() > 0 ){
	    // try {
	    // Job job = jobManager.getJob(assessment.getJob().getJobId());
	...
	if (isStrong)
	    list = assessmentDao.getAssessmentQuestionByJobAndJobLevel(assessment.getJob().getJobId(),
    ...
    @Transactional(readOnly = false, propagation = Propagation.REQUIRES_NEW)
    public void saveOrUpdateUserAssessmentScores(Assessment assessment, User assessor,
	...
	Map<Long, AssessedEssentialElementScoreItem> averages = listToMap(assessmentDao
		.getAssessedEssentialElementScoreAverageByPlanAndJob(assessment.getAssessmentPlan().getAssessmentId(),
			assessment.getJob().getJobId(), assessment.getJobLevel()));

	List<AssessedEssentialElementScore> list = assessmentDao
		.getAssessedEssentialElementSummaries(assessment.getAssessmentId());
	...
	for (AssessedEssentialElementScore summary : list) {
	    ...
	    if (average != null)
		summary.setOthersAverageScore(
			computeAssessedFinalScore(average.getTotalScore(), average.getTotalCount()));
	}
