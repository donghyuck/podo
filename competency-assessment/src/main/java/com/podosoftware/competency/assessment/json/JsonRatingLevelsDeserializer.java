...
public class JsonRatingLevelsDeserializer extends JsonDeserializer<List<RatingLevel>>{
	...
	public List<RatingLevel> deserialize(JsonParser jsonParser, DeserializationContext deserializationContext)
		...
		while( iter.hasNext() ){
			...
			list.add(
				new DefaultRatingLevel(
					child.get("ratingSchemeId").asLong(-1L),
					child.get("ratingLevelId").asLong(-1L),
					child.get("title").asText(),
					child.get("score").asInt(0)					
			));
		}
		/**
		log.debug(list);
		
		Iterator<DefaultRatingLevel> iter2 = oc.readValues(jsonParser, DefaultRatingLevel.class);; //jsonParser.readValuesAs(DefaultRatingLevel.class);
