...
public class JdbcJobDao extends ExtendedJdbcDaoSupport implements JobDao {
     ...
     * 
     * Name Null? Type ----------------------------------------- --------
     * ---------------------------- JOB_ID NOT NULL NUMBER(38) OBJECT_TYPE NOT
     * NULL NUMBER(38) OBJECT_ID NOT NULL NUMBER(38) L_CLASSIFIED_ID NOT NULL
     * NUMBER(38) M_CLASSIFIED_ID NOT NULL NUMBER(38) S_CLASSIFIED_ID NOT NULL
     * NUMBER(38) NAME NOT NULL VARCHAR2(255) DESCRIPTION VARCHAR2(4000)
     * CREATION_DATE NOT NULL TIMESTAMP(6) MODIFIED_DATE NOT NULL TIMESTAMP(6)
     * 
     * 
     */
	...
	public Job mapRow(ResultSet rs, int rowNum) throws SQLException {
	    ...
	    job.setClassification(new DefaultClassification(rs.getLong("CLASSIFY_TYPE"), rs.getLong("L_CLASSIFIED_ID"),
		    rs.getLong("M_CLASSIFIED_ID"), rs.getLong("S_CLASSIFIED_ID")));
	...
	Job job = getExtendedJdbcTemplate().queryForObject(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_JOB_BY_ID").getSql(), jobMapper,
	...
	return getExtendedJdbcTemplate().queryForObject(
		getBoundSql("COMPETENCY_ACCESSMENT.COUNT_JOB_BY_OBJECT_TYPE_AND_OBJECT_ID").getSql(), Integer.class,
	...
	return getExtendedJdbcTemplate().queryForList(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_JOB_ID_BY_OBJECT_TYPE_AND_OBJECT_ID").getSql(), Long.class,
		new SqlParameterValue(Types.NUMERIC, objectType), new SqlParameterValue(Types.NUMERIC, objectId));
    }
	...
	return getExtendedJdbcTemplate().queryScrollable(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_JOB_ID_BY_OBJECT_TYPE_AND_OBJECT_ID").getSql(), startIndex,
		numResults, new Object[] { objectType, objectId }, new int[] { Types.NUMERIC, Types.NUMERIC },
		Long.class);
    }
	 ...
	 * "COMPETENCY_ACCESSMENT.COUNT_JOB_BY_OBJECT_TYPE_AND_OBJECT_ID_AND_CLASSIFY",
	 * classify.toMap()).getSql(), Integer.class, new SqlParameterValue(
	 * Types.NUMERIC, 1), new SqlParameterValue( Types.NUMERIC,
	 ...
	 * return getExtendedJdbcTemplate().queryForList(
	 * getBoundSqlWithAdditionalParameter(
	 * "COMPETENCY_ACCESSMENT.SELECT_JOB_ID_BY_OBJECT_TYPE_AND_OBJECT_ID_AND_CLASSIFY",
	 * classify.toMap()).getSql(), Long.class, new SqlParameterValue(
	 * Types.NUMERIC, 1), new SqlParameterValue( Types.NUMERIC,
	 * company.getCompanyId()) );
	 */
	 ...
	 * getBoundSqlWithAdditionalParameter(
	 * "COMPETENCY_ACCESSMENT.SELECT_JOB_ID_BY_OBJECT_TYPE_AND_OBJECT_ID_AND_CLASSIFY",
	 * classify.toMap()).getSql(), startIndex, numResults, new Object[]{ 1,
	 * company.getCompanyId()}, new int[] {Types.NUMERIC, Types.NUMERIC},
	 * Long.class);
	 */
	...
	return getExtendedJdbcTemplate().queryForObject(
		getBoundSqlWithAdditionalParameter(
			"COMPETENCY_ACCESSMENT.COUNT_JOB_BY_OBJECT_TYPE_AND_OBJECT_ID_AND_CLASSIFY", classify.toMap())
				.getSql(),
    ...
    public List<Long> getJobIds(int objectType, long objectId, Classification classify, int startIndex,
	    int numResults) {
	return getExtendedJdbcTemplate().queryScrollable(
		getBoundSqlWithAdditionalParameter(
			"COMPETENCY_ACCESSMENT.SELECT_JOB_ID_BY_OBJECT_TYPE_AND_OBJECT_ID_AND_CLASSIFY",
	...
	if (inserts.size() > 0) {
	    getExtendedJdbcTemplate().batchUpdate(getBoundSql("COMPETENCY_ACCESSMENT.INSERT_JOB").getSql(),
	...
	return getExtendedJdbcTemplate().queryForList(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_COMPETENCY_ID_BY_JOB_ID").getSql(), Long.class,
		new SqlParameterValue(Types.NUMERIC, job.getJobId()));
    }
	...
	try {
	    jobId = getExtendedJdbcTemplate().queryForObject(
		    getBoundSql("COMPETENCY_ACCESSMENT.SELECT_JOB_ID_BY_COMPETENCY_ID").getSql(), Long.class,
	...
	try {
	    scheme = getExtendedJdbcTemplate().queryForObject(
		    getBoundSql("COMPETENCY_ACCESSMENT.SELECT_JOB_LEVEL_BY_ID").getSql(), jobLevelMapper,
		    new SqlParameterValue(Types.NUMERIC, jobLevelId));
	} catch (IncorrectResultSizeDataAccessException e) {
	    if (e.getActualSize() > 1) {
		log.warn((new StringBuilder()).append("Multiple occurrances of the same JobLevel ID found: ")
			.append(jobLevelId).toString());
		throw e;
	    }
	} catch (DataAccessException e) {
	    String message = (new StringBuilder()).append("Failure attempting to load JobLevel by ID : ")
		    .append(jobLevelId).append(".").toString();
	    log.fatal(message, e);
	}
	return scheme;
    }
	...
	return getExtendedJdbcTemplate().queryForList(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_JOB_LEVEL_IDS_BY_JOB_ID").getSql(), Long.class,
		new SqlParameterValue(Types.NUMERIC, jobId));
    }

    public void removeJobLevels(final List<JobLevel> jobLevels) {
	getExtendedJdbcTemplate().batchUpdate(getBoundSql("COMPETENCY_ACCESSMENT.REMOVE_JOB_LEVEL").getSql(),
		new BatchPreparedStatementSetter() {
		    public void setValues(PreparedStatement ps, int i) throws SQLException {
	...
	if (updates.size() > 0) {
	    getExtendedJdbcTemplate().batchUpdate(getBoundSql("COMPETENCY_ACCESSMENT.UPDATE_JOB_LEVEL").getSql(),
		    new BatchPreparedStatementSetter() {
			public void setValues(PreparedStatement ps, int i) throws SQLException {
	...
	if (inserts.size() > 0) {
	    getExtendedJdbcTemplate().batchUpdate(getBoundSql("COMPETENCY_ACCESSMENT.INSERT_JOB_LEVEL").getSql(),
		    new BatchPreparedStatementSetter() {
			public void setValues(PreparedStatement ps, int i) throws SQLException {
