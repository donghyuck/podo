...
public class JdbcCompetencyDao extends ExtendedJdbcDaoSupport implements CompetencyDao {
	...
	return extendedPropertyDao.getProperties(competencyPropertyTableName, competencyPropertyPrimaryColumnName,
		competencyId);
    }

    public void setCompetencyProperties(long competencyId, Map<String, String> props) {
	extendedPropertyDao.updateProperties(competencyPropertyTableName, competencyPropertyPrimaryColumnName,
		competencyId, props);
    }

    public Map<String, String> getEssentialElementProperties(long essentialElementId) {
	return extendedPropertyDao.getProperties(essentialElementPropertyTableName,
		essentialElementPropertyPrimaryColumnName, essentialElementId);
    }

    public void setEssentialElementProperties(long essentialElementId, Map<String, String> props) {
	extendedPropertyDao.updateProperties(essentialElementPropertyTableName,
		essentialElementPropertyPrimaryColumnName, essentialElementId, props);
    }

    public Map<String, String> getPerformanceCriteriaProperties(long performanceCriteriaId) {
	return extendedPropertyDao.getProperties(this.performanceCriteriaPropertyTableName,
		this.performanceCriteriaPropertyPrimaryColumnName, performanceCriteriaId);
    }

    public void setPerformanceCriteriaProperties(long performanceCriteriaId, Map<String, String> props) {
	extendedPropertyDao.updateProperties(this.performanceCriteriaPropertyTableName,
		this.performanceCriteriaPropertyPrimaryColumnName, performanceCriteriaId, props);
    }

    public void deletePerformanceCriteriaProperties(long performanceCriteriaId) {
	extendedPropertyDao.deleteProperties(this.performanceCriteriaPropertyTableName,
		this.performanceCriteriaPropertyPrimaryColumnName, performanceCriteriaId);
    }
	...
	Competency competency = getExtendedJdbcTemplate().queryForObject(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_COMPETENCY_BY_ID").getSql(), competencyMapper,
	...
	Competency competency = getExtendedJdbcTemplate().queryForObject(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_COMPETENCY_BY_OBJECT_TYPE_AND_OBJECT_ID_AND_NAME").getSql(),
		competencyMapper, new SqlParameterValue(Types.NUMERIC, 1),
	...
	getExtendedJdbcTemplate().update(getBoundSql("COMPETENCY_ACCESSMENT.DELETE_COMPETENCY").getSql(),
		new SqlParameterValue(Types.NUMERIC, competency.getCompetencyId()));
	extendedPropertyDao.deleteProperties(competencyPropertyTableName, competencyPropertyPrimaryColumnName,
		competency.getCompetencyId());
    }

    public int getCompetencyCount(Company company) {
	return getExtendedJdbcTemplate().queryForObject(
		getBoundSql("COMPETENCY_ACCESSMENT.COUNT_COMPETENCY_BY_OBJECT_TYPE_AND_OBJECT_ID").getSql(),
		Integer.class, new SqlParameterValue(Types.NUMERIC, 1),
		new SqlParameterValue(Types.NUMERIC, company.getCompanyId()));
    }

    public List<Long> getCompetencyIds(Company company) {
	return getExtendedJdbcTemplate().queryForList(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_COMPETENCY_ID_BY_OBJECT_TYPE_AND_OBJECT_ID").getSql(),
		Long.class, new SqlParameterValue(Types.NUMERIC, 1),
		new SqlParameterValue(Types.NUMERIC, company.getCompanyId()));
    }
	...
	return getExtendedJdbcTemplate().queryScrollable(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_COMPETENCY_ID_BY_OBJECT_TYPE_AND_OBJECT_ID").getSql(),
		startIndex, numResults, new Object[] { 1, company.getCompanyId() },
	...
	try {
		...
		setEssentialElementProperties(essentialElement.getEssentialElementId(),
			essentialElement.getProperties());
	} catch (DataAccessException e) {
	    throw e;
	}
    }
	...
	EssentialElement essentialElement = getExtendedJdbcTemplate().queryForObject(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_ESSENTIAL_ELEMENT_BY_ID").getSql(), essentialElementMapper,
	...
	return getExtendedJdbcTemplate().queryForList(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_ESSENTIAL_ELEMENT_IDS_BY_COMPETENCY_ID").getSql(), Long.class,
		new SqlParameterValue(Types.NUMERIC, competency.getCompetencyId()));
    }
	...
	if (inserts.size() > 0) {
	    getExtendedJdbcTemplate().batchUpdate(getBoundSql("COMPETENCY_ACCESSMENT.CREATE_COMPETENCY").getSql(),
	...
	return getExtendedJdbcTemplate().query(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_COMPETENCY_ID_WITH_COMPETENCY_UNIT_CODE").getSql(),
	...
	return getExtendedJdbcTemplate().queryForObject(
		getBoundSqlWithAdditionalParameter(
			"COMPETENCY_ACCESSMENT.COUNT_COMPETENCY_BY_OBJECT_TYPE_AND_OBJECT_ID_AND_CLASSIFY",
			classify.toMap()).getSql(),
		Integer.class, new SqlParameterValue(Types.NUMERIC, 1),
		new SqlParameterValue(Types.NUMERIC, company.getCompanyId()));
    }
	...
	return getExtendedJdbcTemplate().queryForList(
		getBoundSqlWithAdditionalParameter(
			"COMPETENCY_ACCESSMENT.SELECT_COMPETENCY_IDS_BY_OBJECT_TYPE_AND_OBJECT_ID_AND_CLASSIFY",
			classify.toMap()).getSql(),
		Long.class, new SqlParameterValue(Types.NUMERIC, 1),
		new SqlParameterValue(Types.NUMERIC, company.getCompanyId()));
    }
	...
	return getExtendedJdbcTemplate().queryForObject(
		getBoundSql("COMPETENCY_ACCESSMENT.COUNT_COMPETENCY_BY_JOB_ID").getSql(), Integer.class,
		new SqlParameterValue(Types.NUMERIC, job.getJobId()));
    }

    public List<Long> getCompetencyIds(Job job) {
	return getExtendedJdbcTemplate().queryForList(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_COMPETENCY_IDS_BY_JOB_ID").getSql(), Long.class,
		new SqlParameterValue(Types.NUMERIC, job.getJobId()));
    }

    public List<Long> getCompetencyIds(Job job, int startIndex, int numResults) {
	return getExtendedJdbcTemplate().queryScrollable(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_COMPETENCY_IDS_BY_JOB_ID").getSql(), startIndex, numResults,
		new Object[] { job.getJobId() }, new int[] { Types.NUMERIC }, Long.class);
    }
	...
	return getExtendedJdbcTemplate().queryForList(
		...
		Long.class, new SqlParameterValue(Types.NUMERIC, job.getJobId()),
	...
	return getExtendedJdbcTemplate().queryForList(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_COMPETENCY_IDS_BY_JOB_ID_AND_LEVEL").getSql(), Long.class,
		new SqlParameterValue(Types.NUMERIC, job.getJobId()), new SqlParameterValue(Types.NUMERIC, level));
    }
	...
	return getExtendedJdbcTemplate().queryForList(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_COMPETENCY_IDS_BY_OBJECT_TYPE_AND_OBJECT_ID_AND_TYPE")
			.getSql(),
    ...
    public List<Long> getCompetencyIds(int objectType, long objectId, CompetencyType competencyType, int startIndex,
	    int numResults) {
	return getExtendedJdbcTemplate().queryScrollable(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_COMPETENCY_IDS_BY_OBJECT_TYPE_AND_OBJECT_ID_AND_TYPE")
			.getSql(),
		startIndex, numResults, new Object[] { objectType, objectId, competencyType.getId() },
		new int[] { Types.NUMERIC, Types.NUMERIC, Types.NUMERIC }, Long.class);
    }

    public List<Long> getCompetencyIds(int objectType, long objectId, String groupCode, int level, String name,
	    Classification classify, long jobId) {
	return getExtendedJdbcTemplate().queryForList(
		getBoundSqlWithAdditionalParameter("COMPETENCY_ACCESSMENT.FIND_COMPETENCY_IDS_BY_INPUT",
			getCompetencyAdditionalParameter(groupCode, level, name, classify, jobId)).getSql(),
		Long.class, new SqlParameterValue(Types.NUMERIC, objectType),
		new SqlParameterValue(Types.NUMERIC, objectId));
    }
	...
	return getExtendedJdbcTemplate().queryScrollable(
		getBoundSqlWithAdditionalParameter("COMPETENCY_ACCESSMENT.FIND_COMPETENCY_IDS_BY_INPUT",
			getCompetencyAdditionalParameter(groupCode, level, name, classify, jobId)).getSql(),
		startIndex, numResults, new Object[] { objectType, objectId },
		new int[] { Types.NUMERIC, Types.NUMERIC }, Long.class);
    }

    @Override
    public int getCompetencyCount(int objectType, long objectId, String groupCode, int level, String name,
	    Classification classify, long jobId) {
	return getExtendedJdbcTemplate().queryForObject(
		getBoundSqlWithAdditionalParameter("COMPETENCY_ACCESSMENT.COUNT_COMPETENCY_IDS_BY_INPUT",
			getCompetencyAdditionalParameter(groupCode, level, name, classify, jobId)).getSql(),
		Integer.class, new SqlParameterValue(Types.NUMERIC, objectType),
		new SqlParameterValue(Types.NUMERIC, objectId));
    }

    private Map<String, Object> getCompetencyAdditionalParameter(String groupCode, int level, String name,
	    Classification classify, long jobId) {
	...
	PerformanceCriteria performanceCriteria = getExtendedJdbcTemplate().queryForObject(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_PERFORMANCE_CRITERIAL_BY_ID").getSql(),
		performanceCriteriaMapper, new SqlParameterValue(Types.NUMERIC, performanceCriteriaId));
	performanceCriteria
		.setProperties(getPerformanceCriteriaProperties(performanceCriteria.getPerformanceCriteriaId()));
	return performanceCriteria;
    }

    public List<Long> getPerformanceCriteriaIds(int objectType, long objectId) {
	return getExtendedJdbcTemplate().queryForList(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_PERFORMANCE_CRITERIA_IDS_BY_OBJECT_TYPE_AND_OBJECT_ID")
			.getSql(),
		Long.class, new SqlParameterValue(Types.NUMERIC, objectType),
		new SqlParameterValue(Types.NUMERIC, objectId));
    }
	...
	try {
		...
		setPerformanceCriteriaProperties(performanceCriteria.getPerformanceCriteriaId(),
			performanceCriteria.getProperties());
	} catch (DataAccessException e) {
	    throw e;
	}
    }
	...
	Ability ability = getExtendedJdbcTemplate().queryForObject(
		getBoundSql("COMPETENCY_ACCESSMENT.SELECT_ABILITY_BY_ID").getSql(), abilityMapper,
	...
	if (inserts.size() > 0) {
	    getExtendedJdbcTemplate().batchUpdate(getBoundSql("COMPETENCY_ACCESSMENT.CREATE_ABILITY").getSql(),
		    new BatchPreparedStatementSetter() {
			public void setValues(PreparedStatement ps, int i) throws SQLException {
	...
	if (updates.size() > 0) {
	    getExtendedJdbcTemplate().batchUpdate(getBoundSql("COMPETENCY_ACCESSMENT.UPDATE_ABILITY").getSql(),
		    new BatchPreparedStatementSetter() {
			public void setValues(PreparedStatement ps, int i) throws SQLException {
	...
	if (deletes.size() > 0) {
	    getExtendedJdbcTemplate().batchUpdate(getBoundSql("COMPETENCY_ACCESSMENT.DELETE_ABILITY").getSql(),
		    new BatchPreparedStatementSetter() {
			public void setValues(PreparedStatement ps, int i) throws SQLException {
